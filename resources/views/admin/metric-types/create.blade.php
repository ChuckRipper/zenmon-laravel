...
<div class="max-w-md mx-auto p-6 space-y-6">
  ...
  <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">➕ Dodaj typ metryki</h1>

  <div class="bg-white dark:bg-gray-800 rounded shadow p-6">
    <form method="POST" action="{{ route('admin.metric-types.store') }}">
      @csrf
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <textarea id="description" name="description" rows="3"
                    class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded p-2
                           bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('description') }}</textarea>
          @error('description')
            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
          @enderror
        </div>
